...
class VentanaModoSemiconductores(Gtk.Window):
    ...
    def on_seleccion(self, selection):
        ...
        # Si no hay dopantes: visor dopado = igual
        if not dopantes:
            self.visor_dopado.actualizar_elemento(z)
            self.leyenda.set_text("No hay dopantes definidos para este elemento.")
            return

        d = dopantes[0]  # elegir uno por defecto
        self.visor_dopado.actualizar_elemento(d)

        # Comparar estructuras
        estructura_pura = ESTRUCTURA_CRISTALINA.get(z, None)
        estructura_dopada = ESTRUCTURA_CRISTALINA.get(d, None)

        if estructura_pura == estructura_dopada:
            self.leyenda.set_text(f"No hay cambio de estructura: ambas son {estructura_pura}.")
        else:
