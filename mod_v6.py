...
# --- Usos comerciales típicos por combinación (semiconductor, dopante) ---
# Clave: (Z_base, Z_dopante)
USOS_COMERCIALES = {
    (14, 5):  "CMOS, sensores analógicos y diodos (Silicio tipo P)",
    (14, 15): "Microprocesadores, memorias DRAM y Flash (Silicio tipo N)",
    (14, 31): "Electrónica de potencia y rectificadores (Si:Ga)",
    (14, 33): "Dispositivos de alta velocidad y RF (Si:As)",

    (32, 5):  "Detectores infrarrojos (Ge tipo P)",
    (32, 15): "Transistores de alta movilidad (Ge tipo N)",

    (31, 33): "Optoelectrónica: LEDs IR, láseres, fotodetectores (GaAs)",
    (31, 51): "Sensores infrarrojos y termofotovoltaicos (GaSb)",

    (49, 33): "Electrónica ultrarrápida y detectores IR (InAs)",
    (49, 51): "Cámaras térmicas y sensores Hall (InSb)",

    (6, 5):   "Electrónica de potencia extrema (Diamante tipo P)",
    (6, 7):   "Detectores de radiación y UV (Diamante tipo N)",
}


DOPANTES = {
    14: [5, 15, 31, 33],     # Si  ← B (p), P (n), Ga, As
    32: [5, 15, 31, 33],     # Ge
    6:  [5, 7],              # Diamante ← B, N
    31: [33, 51],            # Ga ← As, Sb
    33: [31, 49],            # As ← Ga, In
...
class ConfiguracionElectronicaApp(Gtk.Window):
    def __init__(self):
        ...
        # Botón Modo Semiconductores
        self.boton_modo_semic = Gtk.Button(label="Modo Semiconductores")
        self.boton_modo_semic.connect("clicked", self.abrir_modo_semiconductores)
        vbox_izq.pack_start(self.boton_modo_semic, False, False, 0)
    ...
    def abrir_modo_semiconductores(self, widget):
        ventana = VentanaModoSemiconductores()
        ventana.show_all()


class VentanaModoSemiconductores(Gtk.Window):
    def __init__(self):
        super().__init__(title="Modo Semiconductores")
        ...
        # ============================
        vbox_tabla = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=8)
        hbox.pack_start(vbox_tabla, False, False, 0)

        label = Gtk.Label(label="Elementos semiconductores")
        vbox_tabla.pack_start(label, False, False, 0)

        # Modelo: Z (int), símbolo base (str), dopante símbolo (str)
        # Z_base, símbolo_base, Z_dopante, símbolo_dopante
        self.store = Gtk.ListStore(int, str, int, str)

        for z in sorted(SEMICONDUCTORES_INDUSTRIA):
            self.store.append([z, ELEMENTOS[z][0], -1, ""])

        self.treeview = Gtk.TreeView(model=self.store)

        # Columna 1: símbolo del semiconductor
        renderer = Gtk.CellRendererText()
        columna = Gtk.TreeViewColumn("Semiconductor", renderer, text=1)
        self.treeview.append_column(columna)

        # Preparar modelo de dopantes que se actualizará por selección
        self.combo_dopantes_store = Gtk.ListStore(int, str)  # Z, símbolo

        # Columna 2: dopante (editable con CellRendererCombo)
        self.renderer_combo = Gtk.CellRendererCombo()
        self.renderer_combo.set_property("editable", True)
        #self.renderer_combo.set_property("model", self.combo_dopantes_store)
        self.renderer_combo.set_property("text-column", 1)  # símbolo
        self.renderer_combo.set_property("has-entry", False)

        self.renderer_combo.connect("edited", self.on_dopante_editado)

        col_dop = Gtk.TreeViewColumn("Dopante", self.renderer_combo, text=3)
        self.treeview.append_column(col_dop)



        self.treeview.get_selection().connect("changed", self.on_seleccion)

        scrolled = Gtk.ScrolledWindow()
        scrolled.set_size_request(260, 500)
        scrolled.add(self.treeview)
        vbox_tabla.pack_start(scrolled, True, True, 0)

        # Columna dopantes (texto resumen)
        self.dopantes_label = Gtk.Label()
        vbox_tabla.pack_start(self.dopantes_label, False, False, 0)

        # ============================
        # VISOR 1: ESTRUCTURA PURA
        # ============================
        self.visor_puro = VisorCeldillaUnitaria()

        # ============================
        # VISOR 2: ESTRUCTURA DOPADA
        # ============================
        self.visor_dopado = VisorCeldillaUnitaria()

        visores_box = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=10)
        hbox.pack_start(visores_box, True, True, 0)

        visores_box.pack_start(self.visor_puro, True, True, 0)
        visores_box.pack_start(self.visor_dopado, True, True, 0)

        # LEYENDA
        self.leyenda = Gtk.Label()
        self.leyenda.set_line_wrap(True)
        visores_box.pack_start(self.leyenda, False, False, 10)

        # ----------------------------------
        # Leyenda de uso comercial
        # ----------------------------------
        self.uso_comercial_label = Gtk.Label()
        self.uso_comercial_label.set_line_wrap(True)
        self.uso_comercial_label.set_xalign(0)

        self.uso_comercial_label.set_text(
            "Uso comercial: seleccione un semiconductor y un dopante."
        )

        visores_box.pack_start(self.uso_comercial_label, False, False, 10)


    def on_seleccion(self, selection):
        model, treeiter = selection.get_selected()
        if treeiter is None:
            return

        # Semiconductor base
        z_base = model[treeiter][0]
        dopantes = DOPANTES.get(z_base, [])

        # -----------------------------
        # Resumen textual de dopantes
        # -----------------------------
        texto = "Dopantes compatibles: "
        if dopantes:
            texto += ", ".join(ELEMENTOS[d][0] for d in dopantes)
        else:
            texto += "—"
        self.dopantes_label.set_text(texto)

        # -----------------------------
        # Actualizar modelo del combo
        # -----------------------------
        self.combo_dopantes_store.clear()
        for d in dopantes:
            self.combo_dopantes_store.append([d, ELEMENTOS[d][0]])

        # (el renderer ya está asociado, no hace falta reasignarlo,
        # pero no rompe nada si lo dejas)
        self.renderer_combo.set_property("model", self.combo_dopantes_store)

        # -----------------------------
        # Visor de estructura pura
        # -----------------------------
        self.visor_puro.actualizar_elemento(z_base)

        # -----------------------------
        # ¿Ya hay dopante seleccionado?
        # -----------------------------
        z_dop_guardado = model[treeiter][2]      # Z del dopante
        simbolo_dop = model[treeiter][3]         # símbolo del dopante

        if z_dop_guardado > 0 and simbolo_dop:
            # Verificar que siga siendo compatible
            if z_dop_guardado in dopantes:
                self.visor_dopado.actualizar_elemento(z_dop_guardado)
                self.actualizar_leyenda(z_base, z_dop_guardado)
                self.actualizar_uso_comercial(z_base, z_dop_guardado)
                return
            else:
                # Limpiar si ya no es compatible
                model[treeiter][2] = -1
                model[treeiter][3] = ""
        ...
        if dopantes:
            z_def = dopantes[0]
            simbolo_def = ELEMENTOS[z_def][0]

            model[treeiter][2] = z_def
            model[treeiter][3] = simbolo_def

            self.visor_dopado.actualizar_elemento(z_def)
            self.actualizar_leyenda(z_base, z_def)
            self.actualizar_uso_comercial(z_base, z_def)
        else:
            self.visor_dopado.actualizar_elemento(z_base)
            self.leyenda.set_text("No hay dopantes definidos para este elemento.")


    def on_dopante_editado(self, widget, path, new_text):
        # Obtener fila editada
        treeiter = self.store.get_iter(path)
        if treeiter is None:
            return
    
        # Semiconductor base
        z_base = self.store[treeiter][0]
    
        # -----------------------------
        # Caso 1: dopante limpiado
        # -----------------------------
        if not new_text:
            # Limpiar Z y símbolo del dopante
            self.store[treeiter][2] = -1
            self.store[treeiter][3] = ""
    
            # Volver a estructura pura
            self.visor_dopado.actualizar_elemento(z_base)
            self.leyenda.set_text(
                "Dopante removido — estructura vuelve a la del material puro."
            )
            self.uso_comercial_label.set_text(
                "Uso comercial: dopante removido."
            )
        ...
        # Convertir símbolo → Z
        # -----------------------------
        dop_z = SIMBOLO_A_Z.get(new_text)
        if dop_z is None:
        ...
        # -----------------------------
        dopantes_validos = DOPANTES.get(z_base, [])
        if dop_z not in dopantes_validos:
            # Dopante no compatible con este semiconductor
            self.leyenda.set_text(
                f"El dopante {new_text} no es compatible con este semiconductor."
            )
            return
    
        # -----------------------------
        # Guardar selección en la tabla
        # -----------------------------
        self.store[treeiter][2] = dop_z
        self.store[treeiter][3] = new_text
    
        # -----------------------------
        # Actualizar visor y leyenda
        # -----------------------------
        self.visor_dopado.actualizar_elemento(dop_z)
        self.actualizar_leyenda(z_base, dop_z)
        self.actualizar_uso_comercial(z_base, dop_z)


    def actualizar_leyenda(self, z_puro, z_dopado):
        estructura_pura = ESTRUCTURA_CRISTALINA.get(z_puro)
        estructura_dopada = ESTRUCTURA_CRISTALINA.get(z_dopado)
    
        # Tipo de semiconductor (N o P)
        valencia_base = ELEMENTOS[z_puro][1]
        valencia_dop = ELEMENTOS[z_dopado][1]
    
        if valencia_dop > valencia_base:
            tipo = "Tipo N (donador de electrones)"
        elif valencia_dop < valencia_base:
            tipo = "Tipo P (aceptor de electrones)"
        else:
            tipo = "Isoelectrónico (sin dopaje efectivo)"
    
        if estructura_pura == estructura_dopada:
            self.leyenda.set_text(
                f"Estructura sin cambio: {estructura_pura}\n"
                f"Resultado del dopaje: {tipo}."
            )
        else:
            self.leyenda.set_text(
                f"Cambio estructural:\n"
                f"Puro = {estructura_pura}\n"
                f"Dopado = {estructura_dopada}\n"
                f"Resultado del dopaje: {tipo}."
            )

    def actualizar_uso_comercial(self, z_base, z_dop):
        uso = USOS_COMERCIALES.get((z_base, z_dop))

        if uso:
            self.uso_comercial_label.set_text(f"Uso comercial: {uso}")
        else:
            self.uso_comercial_label.set_text(
                "Uso comercial: combinación válida, "
                "pero sin aplicación industrial específica registrada."
            )
