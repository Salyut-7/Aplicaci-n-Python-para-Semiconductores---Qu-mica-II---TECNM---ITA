...
class VentanaModoSemiconductores(Gtk.Window):
    def __init__(self):
        ...
        for z in sorted(SEMICONDUCTORES_INDUSTRIA):
            self.store.append([z, ELEMENTOS[z][0], ""])
        ...
        # model y text-column se ajustarán dinámicamente cuando el usuario seleccione una fila
        self.renderer_combo.set_property("text-column", 1)  # mostrar símbolo (columna 1 del model combo)
        self.renderer_combo.set_property("has-entry", False)
        self.renderer_combo.connect("edited", self.on_dopante_editado)

        col_dop = Gtk.TreeViewColumn("Dopante", self.renderer_combo, text=2)
    ...
    def on_seleccion(self, selection):
        ...
        z = model[treeiter][0]
        dopantes = DOPANTES.get(z, [])

        # Actualizar resumen de dopantes
        texto = "Dopantes compatibles: " + (", ".join(ELEMENTOS[d][0] for d in dopantes) if dopantes else "—")
        self.dopantes_label.set_text(texto)

        # Actualizar modelo del combo con los dopantes compatibles
        self.combo_dopantes_store.clear()
        for d in dopantes:
            self.combo_dopantes_store.append([d, ELEMENTOS[d][0]])

        # Asignar el modelo dinámicamente al renderer del combo
        self.renderer_combo.set_property("model", self.combo_dopantes_store)

        # Visor puro
        self.visor_puro.actualizar_elemento(z)

        # Si la fila ya tenía un dopante seleccionado, actualizar el visor dopado
        dop_actual_simbolo = model[treeiter][2]
        if dop_actual_simbolo:
            dop_z = SIMBOLO_A_Z.get(dop_actual_simbolo)
            if dop_z:
                self.visor_dopado.actualizar_elemento(dop_z)
                self.actualizar_leyenda(z, dop_z)
                return

        # Si no hay dopante seleccionado, mostrar por defecto la primera opción si existe
        if dopantes:
            d = dopantes[0]
            self.visor_dopado.actualizar_elemento(d)
            self.actualizar_leyenda(z, d)
        else:
            self.visor_dopado.actualizar_elemento(z)
            self.leyenda.set_text("No hay dopantes definidos para este elemento.")

    def on_dopante_editado(self, widget, path, new_text):
        # path es la fila editada en self.store
        treeiter = self.store.get_iter(path)
        if treeiter is None:
            return

        z_puro = self.store[treeiter][0]

        # new_text contiene el símbolo seleccionado (porque text-column del combo muestra símbolo)
        if not new_text:
            # limpiar selección
            self.store[treeiter][2] = ""
            self.visor_dopado.actualizar_elemento(z_puro)
            self.leyenda.set_text("Dopante removido — estructura vuelve a la del puro")
            return

        dop_z = SIMBOLO_A_Z.get(new_text)
        if dop_z is None:
            # símbolo desconocido (no debería pasar)
            return

        # Guardar símbolo del dopante en la tabla
        self.store[treeiter][2] = new_text

        # Actualizar visor dopado
        self.visor_dopado.actualizar_elemento(dop_z)

        # Actualizar leyenda
        self.actualizar_leyenda(z_puro, dop_z)
