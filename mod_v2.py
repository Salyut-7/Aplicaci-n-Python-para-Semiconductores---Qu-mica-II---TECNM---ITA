...
class VentanaModoSemiconductores(Gtk.Window):
    def __init__(self):
        ...
        for z in sorted(SEMICONDUCTORES_INDUSTRIA):
            self.store.append([z, ELEMENTOS[z][0], -1])   # sin dopante aún

        # Modelo general para todos los dopantes posibles (se actualiza por fila)
        self.combo_dopantes_store = Gtk.ListStore(int, str)
        # Columna 2: dopantes
        renderer_combo = Gtk.CellRendererCombo()
        renderer_combo.set_property("editable", True)
        renderer_combo.set_property("model", self.combo_dopantes_store)
        renderer_combo.set_property("text-column", 1)
        renderer_combo.set_property("has-entry", False)

        renderer_combo.connect("edited", self.on_dopante_editado)

        col_dop = Gtk.TreeViewColumn("Dopante", renderer_combo, text=2)
    ...
    def on_seleccion(self, selection):
        ...
        # Limpiar modelo del combo
        self.combo_dopantes_store.clear()

        # Llenar con dopantes compatibles
        for d in dopantes:
            self.combo_dopantes_store.append([d, ELEMENTOS[d][0]])

        # Mostrar lista en etiqueta secundaria
        texto = "Dopantes compatibles: " + ", ".join(ELEMENTOS[d][0] for d in dopantes)
        self.dopantes_label.set_text(texto)

        # Actualizar visor puro
        self.visor_puro.actualizar_elemento(z)

        # Si la fila ya tenía un dopante seleccionado, actualizar el visor dopado
        dop_actual = model[treeiter][2]
        if dop_actual > 0:
            self.visor_dopado.actualizar_elemento(dop_actual)
            self.actualizar_leyenda(z, dop_actual)

    def on_dopante_editado(self, widget, path, new_text):
        treeiter = self.store.get_iter(path)
        z = self.store[treeiter][0]

        # Convertir símbolo a Z
        inv = {v[0]: k for k, v in ELEMENTOS.items()}
        if new_text not in inv:
            return

        dop_z = inv[new_text]

        # Guardar dopante seleccionado en la tabla
    ...
    def actualizar_leyenda(self, z_puro, z_dopado):
        ...
        if estructura_pura == estructura_dopada:
            self.leyenda.set_text(
                f"No hay cambio: ambas estructuras son {estructura_pura}."
            )
        else:
            self.leyenda.set_text(
